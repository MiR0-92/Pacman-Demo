...
getScaredTarget(ghost) {
  // Pick a random intersection from the this.intersections array
  let randomIndex = Math.floor(Math.random() * this.intersections.length);
  let randomIntersection = this.intersections[randomIndex];

  // Return the x and y coordinates of the chosen intersection
...
updateGhostPath(ghost,chaseTarget) {
  ...
  preload() {
    ...
    this.load.spritesheet("pinkGhost","/assets/ghost/pink ghost/spr_ghost_pink_0.png",{frameWidth:32,frameHeight:32});
    this.load.spritesheet("orangeGhost","/assets/ghost/orange ghost/spr_ghost_orange_0.png",{frameWidth:32,frameHeight:32});
    this.load.spritesheet("blueGhost","/assets/ghost/blue ghost/spr_ghost_blue_0.png",{frameWidth:32,frameHeight:32});
    this.load.spritesheet("redGhost","/assets/ghost/red ghost/spr_ghost_red_0.png",{frameWidth:32,frameHeight:32});
    this.load.spritesheet("scaredGhost","/assets/ghost/gost afraid/spr_afraid_0.png",{frameWidth:32,frameHeight:32});
    this.load.spritesheet("scaredGhostWhite","/assets/ghost/gost afraid/spr_afraid_1.png",{frameWidth:32,frameHeight:32});

    this.load.spritesheet("pacmanDeath1","/assets/pac man chars/pac man death/spr_pacdeath_0.png",{frameWidth:32,frameHeight:32});
    this.load.spritesheet("pacmanDeath2","/assets/pac man chars/pac man death/spr_pacdeath_1.png",{frameWidth:32,frameHeight:32});
    this.load.spritesheet("pacmanDeath3","/assets/pac man chars/pac man death/spr_pacdeath_2.png",{frameWidth:32,frameHeight:32});

    this.load.image("endGameImage","/assets/pac man text/spr_message_2.png");
    this.load.image("lifeCounter1","/assets/pac man chars/pac man life counter/spr_lifecounter_0.png");
    this.load.image("lifeCounter2","/assets/pac man chars/pac man life counter/spr_lifecounter_0.png");
  ...
  create() {
    ...
    this.socket.on('game-control', (data)=>{
      this.remoteInput[data.direction] = data.pressed;
    });
  ...
  initializeGhosts(layer) {
    this.pinkGhost = this.initializeGhost(232, 280, "pinkGhost", layer);
    this.orangeGhost = this.initializeGhost(210, 280, "orangeGhost", layer);
    this.redGhost = this.initializeGhost(220, 280, "redGhost", layer);
    this.blueGhost = this.initializeGhost(255, 280, "blueGhost", layer);
    this.ghosts = [this.pinkGhost,this.orangeGhost,this.redGhost,this.blueGhost];
    this.startGhostEntries();
    
    this.ghosts = [this.redGhost,this.pinkGhost,this.blueGhost,this.orangeGhost];
    this.ghosts.forEach(ghost =>{
      this.physics.add.overlap(this.pacman,ghost,this.handlePacmanGhostCollision,null,this);
    });
  }
      ...
      for (let x = 0; x < this.map.widthInPixels; x += blockSize) {
        ...
        if (openPaths.length > 2 && y > 64 && y < 530) {
          this.intersections.push({x:x,y:y,openPaths:openPaths});
            ...
            openPaths.length === 2 && y >64 && y< 530) {
              const [dir1,dir2] = openPaths;
              if (((dir1 ==="left" || dir1 === "right") &&
            (dir2 ==="up" || dir2 === "down")) || (dir1 ==="up" || dir1 ==="down") && 
  ...
  update() {
    if(Object.values(this.remoteInput).some(v=>v)){
      this.pacman.setTint(0xffff00);
    } else {
      this.pacman.clearTint();
    }
  ...
  getGhostNextDirection(ghost, intersection) {
    let dx = intersection.x - ghost.x;
    let dy = intersection.y - ghost.y;
  ...
  handleDirectionInput() {
    ...
    for (const dir of directions) {
      if (this.cursors[dir].isDown) {
        this.changeDirectionIfValid(dir);
        return;
      }
    }
    for (const dir of directions){
      if (this.remoteInput[dir]){
        this.changeDirectionIfValid(dir);
        break;
      }
    }
  }
changeDirectionIfValid(newDirection){
  if (this.direction !== newDirection){
    this.previousDirection = this.direction;
    this.direction = newDirection;
    this.nextIntersection = this.getNextIntersectionInNextDirection(this.pacman.x,this.pacman.y,this.previousDirection,newDirection);
  }
}
  ...
  handlePacmanMovement() {
    ...
    const canTurn =
      ...
      ((this.direction === "left" && this.pacman.x > nextIntersectionx) ||
        (this.direction === "right" && this.pacman.x < nextIntersectionx) ||
        (this.direction === "up" && this.pacman.y > nextIntersectiony) ||
        (this.direction === "down" && this.pacman.y < nextIntersectiony));

    if (canTurn || this.isPathOpenAroundPoint(this.pacman.x, this.pacman.y)) {
      switch (this.direction) {
        case "left":
  ...
  handleMovementInDirection(
    ...
    if (condition) {
      ...
      this.adjustPacmanPosition(setVelocityX, setVelocityY);
    } else if (currentVelocity === 0) {
      this.changeDirection(flipX, flipY, angle, setVelocityX, setVelocityY);
      this.adjustPacmanPosition(setVelocityX, setVelocityY);
    }
  ...
  adjustPacmanPosition(setVelocityX, setVelocityY) {
    if (this.pacman.x % this.blockSize !== 0 && setVelocityY > 0) {
      let nearestMultiple =
        Math.round(this.pacman.x / this.blockSize) * this.blockSize;
      this.pacman.body.reset(nearestMultiple, this.pacman.y);
    }
    if (this.pacman.y % this.blockSize !== 0 && setVelocityX > 0) {
      let nearestMultiple =
